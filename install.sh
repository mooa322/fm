...
LICENSES_URL="https://api.github.com/repos/mooa322/fm/contents/licenses.json?ref=main"
...
fm_gate() {
    ...
    local id="${FM_ID:-}"
    if [ -z "$id" ] && [ -f "$FM_LICENSE" ]; then
        id="$(sed -n 's/^FM_ID=//p' "$FM_LICENSE" 2>/dev/null)"
    fi
    if [ -z "$id" ]; then
        echo
        echo -e "  ${C_CYAN}┌── License Required ────────────────────────────────────┐${C_RESET}"
        echo -e "  ${C_CYAN}│${C_RESET}  ${C_WHITE}Enter the license code you received to continue.${C_RESET}"
    ...
    local licenses; licenses="$(curl -fsSL --max-time 10 -H "Accept: application/vnd.github.raw" "$LICENSES_URL" 2>/dev/null || true)"
