...
fm_gate() {
    ...
    rm -f "$enc"
    validate_menu_payload "$FM_SRC/menu.sh" || { rm -rf "$FM_SRC"; echo -e "${C_RED}[FAIL] Decrypted menu payload failed validation.${C_RESET}"; exit 1; }
...
install_tool() {
    ...
    install_menu_payload "$FM_SRC/menu.sh" || exit 1
    patch_menu_in_place "$MENU_BIN"
