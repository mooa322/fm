...
REPO_BASE="https://raw.githubusercontent.com/mooa322/fm/$BRANCH"
...
_fm_pull_src() {
    [ -f "$FM_SRC/menu.sh" ] && [ -f "$FM_SRC/panel/panel.py" ] && return 0
...
cp -f "$FM_SRC/menu.sh" "/usr/local/bin/menu" 2>/dev/null || true
cp -f "$FM_SRC/menu.sh" "/usr/local/bin/firewallfalcon" 2>/dev/null || true
chmod +x "/usr/local/bin/menu" "/usr/local/bin/firewallfalcon" 2>/dev/null || true
sed -i 's/\r$//' "/usr/local/bin/menu" "/usr/local/bin/firewallfalcon" 2>/dev/null || true

# ── Optional: Web Control Panel Backend Update (if service exists) ────────
if [ ! -f "$PANEL_SERVICE" ]; then
    echo
    echo -e "\033[1;32m✅ CLI Tools updated successfully!\033[0m"
    echo -e "\033[1;36mℹ️ You can now run 'menu' directly.\033[0m"
    echo
    exit 0
fi
